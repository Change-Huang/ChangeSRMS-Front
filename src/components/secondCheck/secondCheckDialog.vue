  ...
  <el-dialog title="审核" :visible.sync="showDialog" :before-close="close">
    ...
    <div v-loading="dialogLoading">
      ...
      <el-row :gutter="20" class="row" v-if="showAdminName(history.loanState)">
        <el-col :span="9">
          <div class="title">二审人：</div>
        </el-col>
        <el-col :span="12">
          <div class="detail" v-text="history.superAdminNickname">
          </div>
        </el-col>
      </el-row>
    </div>
...
export default {
  ...
  methods: {
    secondCheckSubmit (id, isPass, version) {
      this.dialogLoading = true
      this.$axios.post('/secondCheck/secondCheckSubmit',
        {
          id: id,
          isPass: isPass,
          version: version
    ...
    // 添加场地表单提交
    pass () {
      this.secondCheckSubmit(this.history.id, true, this.history.version)
    },
    notPass () {
      this.secondCheckSubmit(this.history.id, false, this.history.version)
    },
    close () {
      this.$emit('closeDialog')
    }
